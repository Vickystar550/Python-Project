...
# def watermark_background(draw_object: ImageDraw):
#     # get image size
#     l, h = IMAGE.size
#     mid_l, mid_h = l/2, h/2
#
...
#     y1 = mid_h + 10
#
#     draw_object.rounded_rectangle([x0, y0, x1, y1], radius=3, fill=(128, 128, 128, 0))
...
def write_watermark(draw_object: ImageDraw):
    ...
    if txt_input is not None:
        ...
        canvas_text = canvas.create_text((200, 200), text=f"{txt_input}", fill='white', anchor='s')
...
def picture_watermark():
    ...
    logo_x = int(result_image.width / 2) - 10  # Adjust the position as needed
    logo_y = int(result_image.height / 2) - 10  # Adjust the position as needed
...
orientation_label = Label(text='Change Watermark Orientation', font=('Courier', 20, 'normal'), fg='grey')
orientation_label.grid(row=4, column=0, columnspan=3, pady=20, padx=20)
...
add_water_mark_button.config(padx=30, pady=20, font=('Courier', 20, 'normal'), fg='white')
add_water_mark_button.grid(row=6, column=0)

upload_image_button =  Button(text='Upload Image')
upload_image_button.config(pady=20, padx=30, font=('Courier', 20, 'normal'), fg='white')
upload_image_button.grid(row=6, column=2)

left_orientation_button = Button(text='left')
left_orientation_button.config(font=('Courier', 20, 'normal'), fg='white')
left_orientation_button.grid(row=5, column=0)

right_orientation_button = Button(text='right')
right_orientation_button.config(font=('Courier', 20, 'normal'), fg='white')
right_orientation_button.grid(row=5, column=2)

center_orientation_button = Button(text='center')
center_orientation_button.config(font=('Courier', 20, 'normal'), fg='white')
center_orientation_button.grid(row=5, column=1)

vertical_orientation_button = Button(text='vertical')
vertical_orientation_button.config(font=('Courier', 20, 'normal'), fg='white')
vertical_orientation_button.grid(row=6, column=1)
